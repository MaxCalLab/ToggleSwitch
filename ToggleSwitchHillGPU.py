...
simNum = 9                    # Index of which simulation to infer from
numFits = 1                   # Number of times to use the minimize function for parameter selection
simConditions = 'ToggleHill'  # Name of the simulation conditions
...
fout = 'ExtractedParameters_ToggleGillespieParms_RNA_GPU_timeInc300_'+ str(simConditions) + '_Sim' + str(simNum) + '_SameMaxN_' + str(numIterations) + 'iterations.npz'

""" Filename of progress file """
fprogress = 'ProgressReport_ToggleGillespieParms_RNA_GPU_timeInc300_'+ str(simConditions) + '_Sim' + str(simNum) + '_SameMaxN_' + str(numIterations) + 'iterations.txt'
...
def gillespieSim(conditions,n_A,n_a,n_alpha,n_B,n_b,n_beta):
    ...
    for numTrial in range(max([conditions['numTrials'],len(n_A)])):
        ...
        tempData = open('ProgressReport_ToggleHill_' + str(simNum) + '.txt','a')
...
def conditionsInitHill(g,g_pro,n,K,r,n_A_init,n_B_init,inc,numSteps,numTrials):
    """ Initializes a dictionary containing all of your stated conditions """
    return {'g':g, 'g_pro':g_pro, 'n':n, 'K':K, 'r':r, \
            'N_A_init':n_A_init, 'N_B_init':n_B_init, \
            'inc':inc, 'numSteps':numSteps, 'numTrials':numTrials}

def hillSim(conditions,n_A,n_B):
    """ Gillespie simulation used to generate trajectories using the Hill function propensities """
    """ Needs to run serially, running in parallel produces identical traces, problem with random seed... """
    if len(n_A) == 0:
        n_A = (conditions['N_A_init']*np.ones((conditions['numTrials'],1))).tolist()
        n_B = (conditions['N_B_init']*np.ones((conditions['numTrials'],1))).tolist()
    for numTrial in range(max([conditions['numTrials'],len(n_A)])):
        ...
        tempData.close()
        while timeFrame < float(conditions['numSteps']*conditions['inc']):
            prob = [conditions['g'] + conditions['g_pro']*1/(numB**conditions['n'] + conditions['K']),\
                    conditions['g'] + conditions['g_pro']*1/(numA**conditions['n'] + conditions['K']),\
                    conditions['r']*numA,conditions['r']*numB]
            overallRate = sum(prob)
            randNum1 = np.random.rand(1)
            timeFrame -= np.log(randNum1)/overallRate
            while timeFrame >= incCheckpoint:
            ...
            prob = prob/overallRate
            randNum2 = np.random.rand(1)
            if randNum2 <= sum(prob[:1]):
                numA += 1
            elif randNum2 <= sum(prob[:2]):
                numB += 1
...
def hillFSP_mle(reactionRates):
    ...
    """ an input trajectory occurring in a Hill function reaction schema. """
    """ reactionRates = ['g','g_pro','K','r'] """
    global probs
    global maxN
    ...
    if np.any(np.array(reactionRates) < 0):
        return float('Inf')
    probMatrix = np.zeros((maxN**2,maxN**2))
    for probN_A in range(maxN):
        for probN_B in range(maxN):
            probMatrix[probN_A*maxN + probN_B][probN_A*maxN + probN_B] -= \
            (reactionRates[0] + reactionRates[1]/(probN_B**n + reactionRates[2]) + \
            reactionRates[0] + reactionRates[1]/(probN_A**n + reactionRates[2]) + \
            reactionRates[3]*probN_A + reactionRates[3]*probN_B)
            if probN_A < maxN - 1:
                probMatrix[(probN_A + 1)*maxN + probN_B][probN_A*maxN + probN_B] += \
                reactionRates[0] + reactionRates[1]/(probN_B**n + reactionRates[2])
            if probN_A > 0:
                probMatrix[(probN_A - 1)*maxN + probN_B][probN_A*maxN + probN_B] += reactionRates[3]*probN_A
            if probN_B < maxN - 1:
                probMatrix[probN_A*maxN + probN_B + 1][probN_A*maxN + probN_B] += \
                reactionRates[0] + reactionRates[1]/(probN_A**n + reactionRates[2])
            if probN_B > 0:
                probMatrix[probN_A*maxN + probN_B - 1][probN_A*maxN + probN_B] += reactionRates[3]*probN_B
    probMatrix = np.hstack((probMatrix,np.zeros((maxN**2,1))))
    probMatrix = np.vstack((probMatrix,-1*np.sum(probMatrix,axis=0)))
    startTime = datetime.datetime.now()    
    numSteps = int(timeInc*numIterations)
    result_gpu = cudaExp(probMatrix*numSteps)
    endTime = datetime.datetime.now()
    timeVals.append((endTime - startTime).total_seconds())
    result_gpu = cp.asnumpy(result_gpu)
    loglike = -1*np.nansum(np.nansum(np.log(result_gpu)*probs.toarray()))
    tempData = open('ProgressReport_ToggleHill_' + str(simNum) + '.txt','a')
    tempData.write('g = ' + str(round(reactionRates[0],3)) + ', g_pro = ' + str(round(reactionRates[1],3)) + \
    ', K = ' + str(round(reactionRates[2],4)) + ', r = ' + str(round(reactionRates[3],4)) + \
    ', n = ' + str(n) + ', loglike = ' + str(round(loglike,1)) + '\n')
    tempData.close()
    return loglike
...
for numFit in range(numFits):
    ...
    tempData.close()
    startGuess.append((0.6 + 0.8*np.random.rand())*np.array([1.5e-07, 3.8, 32, 0.007]))
    res = minimize(hillFSP_mle,startGuess[numFit],method='nelder-mead',tol=0.01,options={'disp':True,'maxiter':500})
